...
public class ProgramasController {
    ...
    private void initialize() {
        ...
        typeGroup.selectedToggleProperty().addListener((obs, oldToggle, newToggle) -> {
            ...
            if (nuevoTipo != tipoActivo) {
                ...
                String filtro = nuevoTipo == ProgramaTipo.APLICACION
                        ? "Aplicaciones"
    ...
    private void startScan(boolean manualRefresh) {
        ...
        if (manualRefresh) {
            historialService.addEvent(
                    "Escaneo actualizado",
                    "Actualización manual del escaneo de programas",
                    HistorialService.CATEGORIA_PROGRAMAS
            );
        } else {
            historialService.addEvent(
                    "Escaneo iniciado",
                    "Escaneo de programas instalados en Windows",
        ...
        scanTask.setOnFailed(event -> {
            ...
            historialService.addEvent(
                    "Error de escaneo",
                    "No se pudo completar el escaneo de programas",
    ...
    private void handleScanResult(ScanResult result) {
        if (result.ok()) {
            ...
            historialService.addEvent(
                    "Escaneo completado",
                    total + " programas detectados (" + apps + " aplicaciones, " + system + " sistema)",
                    HistorialService.CATEGORIA_PROGRAMAS
            );
            refreshList();
            return;
